...
- Streams Gemini responses via OpenRouter per page (optional --stream)
...
- Provider: Gemini models accessed through OpenRouter's chat completions API.
"""
from __future__ import annotations

import argparse, json, logging, os, re, sys, time, shutil, itertools, base64, mimetypes
...
class OpenRouterAdapter(ProviderAdapter):
    def __init__(self):
        ...
        self._api_base: str = "https://openrouter.ai/api/v1"
        self._referer: Optional[str] = None
        self._app_title: Optional[str] = None
    ...
    def init(
        ...
        base = api_base or os.getenv("OPENROUTER_API_BASE") or self._api_base
        self._api_base = base.rstrip("/")
        ref = referer or os.getenv("OPENROUTER_APP_URL") or os.getenv("OPENROUTER_HTTP_REFERER")
        title = app_title or os.getenv("OPENROUTER_APP_TITLE")
        timeout_env = os.getenv("OPENROUTER_TIMEOUT")
        if timeout_env:
            try:
                self.timeout = max(1, int(timeout_env))
            except ValueError:
                LOG.warning("Invalid OPENROUTER_TIMEOUT=%s; using default %d", timeout_env, self.timeout)
        self._referer = ref
    ...
    def reconfigure(self, api_key: str) -> None:
        """Reconfigure underlying session with a new API key (for rotation)."""

        self.init(api_key, api_base=self._api_base, referer=self._referer, app_title=self._app_title)

    def build_model(self, model_name: str, schema: Optional[dict], max_output_tokens: int) -> None:
    ...
    def make_file_part(self, path: Path) -> Dict[str, Any]:
    ...
    def _extract_text_from_content(self, content: Any) -> str:
        ...
        if isinstance(content, list):
            pieces: List[str] = []
            for item in content:
                pieces.append(self._extract_text_from_content(item))
            return "".join(pieces)
    ...
    def _extract_text_from_delta(self, delta: Any) -> str:
        if delta is None:
            return ""
        if isinstance(delta, dict):
            if delta.get("content") is not None:
                return self._extract_text_from_content(delta.get("content"))
    ...
    def _extract_text_from_message(self, message: Any) -> str:
        ...
        if hasattr(message, "content"):
            return self._extract_text_from_content(getattr(message, "content"))
        return ""

    def generate_stream_or_text(self, msgs: list, stream: bool) -> str:
        if not self.session or not self.model_name:
            raise RuntimeError("OpenRouter adapter is not initialized.")
        ...
        if stream:
            ...
            if resp.status_code >= 400:
                body = resp.text
                resp.close()
                raise RuntimeError(f"OpenRouter error {resp.status_code}: {body[:400]}")
            acc: List[str] = []
            try:
                for line in resp.iter_lines(decode_unicode=True):
                    ...
                    for choice in choices:
                        ...
                        text = self._extract_text_from_delta(delta)
                        if text:
                            acc.append(text)
        ...
        try:
            ...
            raise RuntimeError(f"OpenRouter returned non-JSON response: {resp.text[:400]}")
...
# -------------------- robust API key load (OpenRouter) -------------
def load_api_key(cli_key: Optional[str]) -> str:
    ...
    if not api_key:
        LOG.critical(
            "Missing API key. Provide via --api_key, env OPENROUTER_API_KEY, .env, or openrouter_key.txt/key.txt"
        )
        raise SystemExit(1)
    return api_key
...
def load_api_key_pool(primary: Optional[str]) -> List[str]:
    ...
    Sources (in order):
      ...
      2. openrouter_apikeys.txt / openrouter_keys.txt / apikey.txt (one key per line)
      3. OPENROUTER_API_KEY env / openrouter_key.txt fallback (if no pool file)
    ...
    keys: List[str] = []
    pool_candidates = [
        Path("openrouter_apikeys.txt"),
        Path("openrouter_keys.txt"),
        Path("apikey.txt"),
    ]
    pool_file = next((p for p in pool_candidates if p.exists()), None)
    if pool_file:
...
def stream_or_generate_json(adapter, prompt: str, img_path: Path, page_number: int, stream: bool, show_prompt: bool=False) -> str:
    ...
    msgs = [{"role": "user", "content": [{"type": "text", "text": full_prompt}, file_part]}]
    if stream:
...
def stream_or_generate_json_dual(adapter, prompt: str, img_path1: Path, page_number1: int, img_path2: Path, page_number2: int, stream: bool, show_prompt: bool=False) -> str:
    ...
    msgs = [{"role": "user", "content": [{"type": "text", "text": full_prompt}, file_part1, file_part2]}]
    if stream:
...
def main():
    ap = argparse.ArgumentParser(
        description="Cached images → Gemini via OpenRouter with streaming, continuation, retries — RESUMABLE."
    )
    # Common
    ...
    ap.add_argument("--model", type=str, default="google/gemini-1.5-pro-latest")
    ...
    ap.add_argument("--api_base", type=str, default=None, help="Override OpenRouter API base URL.")
    ap.add_argument("--http_referer", type=str, default=None, help="Optional HTTP-Referer header value for OpenRouter.")
    ap.add_argument("--app_title", type=str, default=None, help="Optional X-Title header value for OpenRouter.")
    args = ap.parse_args()
    ...
    if not api_keys:
        ...
        raise SystemExit(1)
    adapter: ProviderAdapter = OpenRouterAdapter()
    active_key_index = 0
    adapter.init(
        api_key=api_keys[active_key_index],
    ...
    while i < len(pages):
        ...
        while attempt < args.max_attempts and not success:
            ...
            try:
                if dual:
                    LOG.info("[OPENROUTER] Pages %d+%d attempt %d/%d", pg1, pg2, attempt, args.max_attempts)
                    raw = stream_or_generate_json_dual(adapter, prompt, img1, pg1, img2, pg2, stream=args.stream, show_prompt=args.show_prompt)
                else:
                    LOG.info("[OPENROUTER] Page %d attempt %d/%d", pg1, attempt, args.max_attempts)
