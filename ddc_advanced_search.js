...
class DDCAdvancedSearch {
    constructor() {
        ...
        this.notationIndex = new Map(); // notation -> node_id
    ...
    searchNotation(query, maxResults = 50) {
        ...
        if (this.notationIndex.has(queryLower)) {
            const nodeId = this.notationIndex.get(queryLower);
            results.set(nodeId, 1000.0); // Very high score for exact match
        }
        
        // 2. Notation starts with query
        for (const [notation, nodeId] of this.notationIndex) {
    ...
    searchKeyword(query, maxResults = 50) {
        ...
        if (this.notationIndex.has(query.toLowerCase())) {
            const nodeId = this.notationIndex.get(query.toLowerCase());
    ...
    loadData(metaData) {
        ...
        for (const [nodeId, entry] of Object.entries(metaData)) {
            ...
            if (!notation && /^\d/.test(nodeId)) {
                // If no notation property, use nodeId if it looks like a DDC notation
                notation = nodeId;
            }
            if (notation) {
                this.notationIndex.set(notation.toLowerCase(), nodeId);
            }
    ...
    getSuggestions(partialQuery, maxSuggestions = 10) {
        ...
        for (const notation of this.notationIndex.keys()) {
            if (notation.startsWith(partialNorm)) {
                const nodeId = this.notationIndex.get(notation);
                const entry = this.data[nodeId];
                if (entry?.notation) {
    ...
    filterResultsByTable(results, tableFilter) {
        ...
        return results.filter(([nodeId, score, matchType]) => {
            const entry = this.data[nodeId] || {};
            let nodeTable = entry.table || '';
            
            // Handle T3 subtables (T3A, T3B, T3C should match T3)
            if (tableFilter === 'T3' && nodeTable.startsWith('T3')) {
                nodeTable = 'T3';
            }
