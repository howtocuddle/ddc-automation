...
- Streams Mistral responses per page (optional --stream)
...
- Provider: Mistral AI via the mistralai Python SDK.
"""
...
class MistralAdapter(ProviderAdapter):
    ...
    def generate_stream_or_text(self, msgs: list, stream: bool) -> str:
        if not self.client or not self.model_name:
            raise RuntimeError("Mistral adapter is not initialized.")
        ...
        if stream:
            acc: List[str] = []
            stream_resp = self.client.chat.stream(**params)
            for event in stream_resp:
                ...
                for choice in choices:
                    ...
                    if not delta:
                        continue
                    text = self._extract_text_from_content(getattr(delta, "content", None))
                    if text:
                        acc.append(text)
        ...
        return self._extract_text_from_content(getattr(message, "content", None))

"""Vertex adapter removed."""

# -------------------- robust API key load (Mistral) -------------
def load_api_key(cli_key: Optional[str]) -> str:
    if load_dotenv:
        try: load_dotenv()
        except Exception: pass
    api_key = cli_key or os.getenv("MISTRAL_API_KEY")
    if not api_key:
        key_file = Path("mistral_key.txt")
        if key_file.exists():
    ...
    if not api_key:
        ...
        if key_file.exists():
            api_key = key_file.read_text(encoding="utf-8").strip()
    if not api_key:
        LOG.critical("Missing API key. Provide via --api_key, env MISTRAL_API_KEY, .env, or mistral_key.txt/key.txt")
        raise SystemExit(1)
    return api_key
...
def load_api_key_pool(primary: Optional[str]) -> List[str]:
    ...
    Sources (in order):
      ...
      2. mistral_apikeys.txt or apikey.txt (one key per line, blank/comment lines ignored)
      3. MISTRAL_API_KEY env / mistral_key.txt fallback (if no pool file)
    Duplicates removed preserving order.
    """
    if primary:
        return [primary.strip()]
    keys: List[str] = []
    pool_candidates = [Path("mistral_apikeys.txt"), Path("apikey.txt")]
...
# --------------------------- main -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Cached images → Mistral (mistralai API) with streaming, continuation, retries — RESUMABLE.")
    # Common
    ...
    ap.add_argument("--model", type=str, default="pixtral-large-latest")
    ...
    if not api_keys:
        ...
        raise SystemExit(1)
    adapter: ProviderAdapter = MistralAdapter()
    active_key_index = 0
    ...
    while i < len(pages):
        ...
        while attempt < args.max_attempts and not success:
            ...
            try:
                if dual:
                    LOG.info("[MISTRAL] Pages %d+%d attempt %d/%d", pg1, pg2, attempt, args.max_attempts)
                    raw = stream_or_generate_json_dual(adapter, prompt, img1, pg1, img2, pg2, stream=args.stream, show_prompt=args.show_prompt)
                else:
                    LOG.info("[MISTRAL] Page %d attempt %d/%d", pg1, attempt, args.max_attempts)
